...
rank, as of 2026-09-14. The two entries above it belong to the
leaderboard's agentic category, multi-step systems that use agents or
language models to reason about, generate or select forecasts, and both
run a large language model. TW3Cast runs none. Its selection is a table
computed once on the training split and then frozen, its experts are
public foundation models lightly fine-tuned on those training splits. For each of the 97 dataset,
...
generate or select forecasts. The two entries above ours in
Section~\ref{sec:results} are documented. STRIDE
\citep{ahamed2026stride} trains a time-series foundation model on
reasoning traces produced by a language model, and Synapse
\citep{das2025synapse} arbitrates between several foundation models with
weights computed at inference. EXAONE-Forecast-Agent \citep{exaoneforecast}
wraps a forecasting model in a multi-agent pipeline driven by a large
language model. The lower agentic entries have no public description we
could read. TW3Cast shares with these systems the idea of routing among
public foundation models per data source, and differs in the three
choices above: no language model anywhere, a routing decided once on the
training split and frozen, and experts obtained by light fine-tuning.
...
Blends are sorted quantile means across architectures, so that the
members of a blend fail on different windows. Each candidate is a checkpoint of a few megabytes trained
...
checkpoints and one GPU. The tournament procedure
